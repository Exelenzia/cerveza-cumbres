    ...
    <body class="min-h-screen bg-surface font-sans text-text antialiased transition-colors duration-300">
        ...
        <header class="sticky top-0 z-40 border-b border-border bg-surface/95 backdrop-blur">
            <div class="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
                ...
                <nav class="hidden items-center gap-8 font-display text-sm font-medium tracking-widest text-text/90 uppercase md:flex">
                ...
                <div class="flex items-center gap-3 font-display text-sm font-medium tracking-widest uppercase">
                    ...
                    @auth
                        ...
                        <span class="hidden text-text/60 sm:inline">Hola, {{ explode(' ', auth()->user()->name)[0] }}</span>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="rounded border border-border px-4 py-2 text-text transition hover:border-primary hover:text-primary">Salir</button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" wire:navigate class="text-text/80 transition hover:text-primary">Ingresar</a>
                        <a href="{{ route('register') }}" wire:navigate class="rounded border border-primary px-4 py-2 text-primary transition hover:bg-primary hover:text-primary-on">Crear cuenta</a>
                    @endauth
                    <button
                        type="button"
                        data-theme-toggle
                        class="inline-flex items-center gap-2 rounded-full border border-border bg-surface-elevated px-3 py-1.5 normal-case tracking-normal transition hover:border-primary"
                        aria-label="Cambiar tema"
                    >
                        <span data-theme-icon class="grid h-5 w-5 place-items-center rounded-full bg-primary text-[.7rem] text-primary-on">☾</span>
                        <span data-theme-label class="hidden text-xs text-text sm:inline">Oscuro</span>
                    </button>
                </div>
            </div>
        </header>
        ...
        <footer class="border-t border-border bg-surface-muted">
            <div class="mx-auto grid max-w-6xl gap-10 px-6 py-14 md:grid-cols-4">
                ...
                <div>
                    <p class="font-display text-sm font-semibold tracking-widest text-secondary uppercase">Navegación</p>
                ...
                <div>
                    <p class="font-display text-sm font-semibold tracking-widest text-secondary uppercase">Delivery Perú</p>
                    <p class="mt-4 text-sm text-text/70">Enviamos a Lima en 24h y al interior del país en 24–72h.</p>
                </div>

                <div>
                    <p class="font-display text-sm font-semibold tracking-widest text-secondary uppercase">Contacto</p>
                    <ul class="mt-4 space-y-2 text-sm text-text/70">
            ...
            <div class="border-t border-border px-6 py-6 text-center text-xs text-text-muted">
