            ...
            <form wire:submit="save" class="space-y-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" wire:model="code" placeholder="CUMBRES10" class="w-full rounded-lg border border-border bg-surface px-3 py-2 uppercase text-text focus:border-primary focus:outline-none">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" wire:model="value" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    ...
                    <div>
                        ...
                        <select wire:model.live="scope" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    ...
                    @if ($scope === 'product')
                        <div>
                            ...
                            <select wire:model="product_id" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" wire:model="min_order_amount" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    </div>
                    <div>
                        <label class="mb-1 block text-sm text-text-muted">Usos máximos (opcional)</label>
                        <input type="number" wire:model="max_uses" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    </div>
                    <div>
                        <label class="mb-1 block text-sm text-text-muted">Fecha de expiración (opcional)</label>
                        <input type="date" wire:model="expires_at" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    </div>
                </div>
                <label class="flex items-center gap-2">
                    <input type="checkbox" wire:model="is_active" class="h-5 w-5 rounded border-border bg-surface text-primary">
            ...
            <tbody class="divide-y divide-border">
                @forelse ($coupons as $coupon)
                    <tr class="bg-surface/40">
                        ...
                        <td class="px-4 py-3">
                            <span class="rounded-full px-2 py-1 text-xs {{ $coupon->is_active ? 'bg-green-500/20 text-green-300' : 'bg-surface-muted text-text-muted/60' }}">
