            ...
            <form wire:submit="save" class="space-y-4">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select wire:model="type" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                ...
                <div>
                    ...
                    <textarea wire:model="description" rows="2" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none"></textarea>
                </div>

                <div class="grid grid-cols-2 gap-4 md:grid-cols-3">
                    <div>
                        <label class="mb-1 block text-sm text-text-muted">Unidades de cerveza</label>
                        <input type="number" min="1" wire:model="bottle_count" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" wire:model="base_price" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                ...
                @if ($type === 'gift')
                    <div>
                        ...
                        <select wire:model="included_merch_product_id" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="number" step="0.01" wire:model="delivery_cost" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="delivery_note" placeholder="Ej. Sujeto a disponibilidad de ruta" class="w-full rounded-lg border border-border bg-surface px-3 py-2 text-text focus:border-primary focus:outline-none">
                    </div>
                </div>

                <label class="flex items-center gap-2 text-sm text-text-muted">
                    <input type="checkbox" wire:model="free_shipping_eligible" class="rounded border-border bg-surface text-primary">
                ...
                <div>
                    ...
                    <div class="grid max-h-64 grid-cols-1 gap-2 overflow-y-auto rounded-lg border border-border bg-surface p-3 sm:grid-cols-2">
                        @foreach ($products as $product)
                            <label class="flex items-center gap-2 text-sm text-text-muted">
                                <input type="checkbox" wire:model="eligible_product_ids" value="{{ $product->id }}" class="rounded border-border bg-surface text-primary">
                                {{ $product->name }}
                            </label>
                        @endforeach
                    </div>
