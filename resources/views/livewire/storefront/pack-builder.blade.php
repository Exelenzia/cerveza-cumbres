<div class="mx-auto max-w-5xl px-6 py-16">
    <div class="mb-10 text-center">
        ...
        <h1 class="mt-2 font-display text-4xl uppercase tracking-wide text-text">{{ $template->name }}</h1>
    ...
    @if ($includedMerch)
        <div class="mb-6 rounded-lg border border-primary/40 bg-primary/10 px-4 py-3 text-center text-sm text-primary">
            Incluye gratis: {{ $includedMerch->name }}
        </div>
    @endif

    @if ($template->delivery_note)
        <div class="mb-6 rounded-lg border border-border bg-surface-muted px-4 py-3 text-center text-sm text-text/70">
    ...
    <div class="mb-6 flex items-center justify-between rounded-lg border border-border bg-surface-muted px-5 py-4">
        <p class="text-text">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($eligibleProducts as $product)
                ...
                <button
                    ...
                    class="rounded-xl border p-5 text-left transition {{ $selected ? 'border-primary bg-primary/10' : 'border-border bg-surface-muted hover:border-primary/50' }}"
                >
                    ...
                    @if ($selected)
                        <span class="mt-3 inline-block rounded-full bg-primary px-3 py-1 text-xs font-semibold text-primary-on">Elegido</span>
                    @endif
                </button>
            @endforeach
        </div>
            ...
            @foreach ($eligibleProducts as $product)
                ...
                <div class="rounded-xl border border-border bg-surface-muted p-5">
                    ...
                    <div class="mt-4 flex items-center justify-between">
                        <button type="button" wire:click="decrement({{ $product->id }})" class="h-8 w-8 rounded-lg border border-border text-text hover:border-primary">−</button>
                        <span class="font-semibold text-text">{{ $qty }}</span>
                        <button type="button" wire:click="increment({{ $product->id }})" class="h-8 w-8 rounded-lg border border-border text-text hover:border-primary">+</button>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="mt-10 flex justify-center">
        <button
            ...
            class="rounded-lg px-8 py-3 text-sm font-semibold transition {{ $totalUnits === $template->bottle_count ? 'bg-primary text-primary-on hover:bg-primary-hover' : 'cursor-not-allowed bg-surface-muted text-text/50' }}"
        >
            Agregar al carrito
        </button>
    </div>
