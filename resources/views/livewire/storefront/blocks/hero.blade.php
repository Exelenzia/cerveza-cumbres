...
<section class="relative overflow-hidden bg-surface">
    <div class="mx-auto flex max-w-7xl flex-col items-center px-6 py-24 text-center sm:py-32">
        ...
        @endif
        <h1 class="font-display text-4xl uppercase tracking-wide text-text sm:text-6xl">
            {{ $data['heading'] ?? '' }}
        </h1>
        @if (! empty($data['subheading']))
            <p class="mt-6 max-w-xl text-lg text-text/80">{{ $data['subheading'] }}</p>
        @endif
        @if (! empty($data['button_label']) && ! empty($data['button_link']))
            <a href="{{ $data['button_link'] }}" class="mt-10 inline-block rounded-lg bg-primary px-8 py-3 font-semibold text-primary-on hover:bg-primary-hover">
                {{ $data['button_label'] }}
            </a>
        @endif
    </div>
