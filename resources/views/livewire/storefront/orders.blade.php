<div class="mx-auto max-w-4xl px-6 py-16">
    <h1 class="mb-10 font-display text-4xl uppercase tracking-wide text-text">Mis pedidos</h1>
    ...
    @else
        <div class="space-y-4">
            @foreach ($orders as $order)
                <a href="{{ route('orders.show', $order) }}" wire:navigate class="flex items-center justify-between rounded-xl border border-border bg-surface-muted p-5 transition hover:border-primary">
                    <div>
                    ...
                    <div class="text-right">
                        ...
                        <span class="mt-1 inline-block rounded-full bg-primary/20 px-3 py-1 text-xs text-primary">{{ $order->statusLabel() }}</span>
                    </div>
                </a>
            @endforeach
        </div>
